// Add the ability for the user to return to the menu page from Level 2 easily
...
// Loads to JSON data which contains all the card names and images, it then picks 8 random pairs (in total 16 cards)
// fetch() is used to keep the card data in a separate JSON file, which makes it easier to use across multiple levels
fetch("./data/cards.json")
  .then((res) => res.json())
  .then((data) => {
    const selectedCards = getRandomCards(data, 8)
...
function restart() {
  ...
  timeLeft = 35; // Resets the countdown back to the original number
...
function showEndPopup(score) {
  ...
  // A Win is when the score is from 8 to 14
  if (score >= 8 && score <= 14) {
    winSound.play();
    popupWin.style.display = 'block';
    buttonWin.onclick = () => window.location.href = "level3.html"; // Navigates to the Hard Level when the "Advance" button is clicked
  } 
  // An Average is when the score is from 15 to 22
    ...
    buttonAdvance.onclick = () => window.location.href = "level3.html"; // Navigates to the Hard Level when the "Advance" button is clicked
  } 
  // A loss is when the score is 23 or higher
  else if (score >= 23 ) {
    failSound.play();
    popupFail.style.display = 'block';    
    buttonFail.onclick = () => window.location.href = "level1.html"; // Navigates to the Easy Level when the "Go down" button is clicked
  }
...
// Sets the timer to be 35 seconds
let timeLeft = 35
let timeInterval;
...
gameEnded = true; // Stops the user from flipping the cards again
  ...
  if (result === "fail") {
    ...
    const buttonFail = document.getElementById("buttonFail");
    buttonFail.onclick = () => {
      window.location.href = "level1.html"; // Sends the user back to the Easy level if the "Go down a Level" button is clicked
    }
  }
}
