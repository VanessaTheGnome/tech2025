// Add the ability for the user to return to the menu page from Level 1 easily
...
// Loads to JSON data which contains all the card names and images, it then picks 4 random pairs (in total 8 cards)
// fetch() is used to keep the card data in a separate JSON file, which makes it easier to use across multiple levels
fetch("./data/cards.json")
  .then((res) => res.json())
  .then((data) => {
    const selectedCards = getRandomCards(data, 4)
...
// I chose a 3-second time bonus to reward the user, because it wasn't too large or too small
...
function restart() {
  ...
  timeLeft = 30; // Resets the countdown back to the original number
...
function showEndPopup(score) {
  ...
  // A Win is when the score is from 4 to 7
  if (score >= 4 && score <= 7) {
    winSound.play();
    popupWin.style.display = 'block';
    buttonWin.onclick = () => window.location.href = "level2.html"; // Navigates to the Medium Level when the "Advance" button is clicked
  } 
  // An Average is when the score is from 8 to 10
    ...
    buttonAdvance.onclick = () => window.location.href = "level2.html"; // Navigates to the Medium level when the "Advance" button is clicked
  } 
  // A Loss is when the score is 11 or higher
  else if (score >= 11 ) {
    failSound.play();
    popupFail.style.display = 'block';    
    buttonFail.onclick = () => restart(); // Restarts the entire game if the "Try Again" button is clicked
  }
...
// Sets the timer to be 30 seconds
let timeLeft = 30
let timeInterval;
...
function endgame(result) {
  ...
  if (result === "fail") {
    ...
    const buttonFail = document.querySelector(".buttonFail");
    buttonFail.onclick = () => {
      document.querySelector(".popupFail").style.display = "none"
      restart(); // If the "Try Again" button is pressed it runs the restart function and restarts the entire game
    }
  }
}
